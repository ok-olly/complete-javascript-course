...
// 이렇게 해도 작동은 하는데 Number.parse어쩌구로 쓰는 것을 추천함
...
// 10픽셀 반지름을 가진 원의 넓이 구하기
console.log(Math.PI * Number.parseFloat('10px') ** 2);

// 1부터 6까지의 랜덤한 정수를 출력하기
// Math.trunc(Math.random() * 6)를 하면 5까지만 출력이 되기 때문에 1을 더해준다.
console.log(Math.trunc(Math.random() * 6) + 1);

// 내가 정한 두 숫자 사이의 랜덤한 정수 뽑아내기
...
// 1일을 밀리세컨즈로 환산하려면 1초는 1000 밀리세컨즈 그리고 60초, 60분, 24시간 순으로 곱해준다.
// (1000 * 60 * 60 * 24)
// 이걸 두 날짜간의 차이인 (date2 - date1) 값에 위에서 계산한 하루의 밀리세컨즈를 나누면 열흘이라는 값이 계산된다.
const calcDaysPassed = (date1, date2) =>
...
const options = {
  ...
  // unit: 'mile-per-hour',
  unit: 'celsius',
  // currency는 locale로 정해지는 것이 아니기 때문에 여기에 직접 원하는 통화를 적어줘야한다.
  currency: 'EUR',
  // useGrouping을 false로 지정하면 천단위 구분기호가 사라진다.
  // useGrouping: false,
};
